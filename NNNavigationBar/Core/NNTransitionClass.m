...
static pthread_mutex_t nn_nav_injectLock = PTHREAD_MUTEX_INITIALIZER;
...
void nn_nav_loadSection(const struct mach_header* mhp, intptr_t vmaddr_slide) {
    
    if (pthread_mutex_lock(&nn_nav_injectLock) != 0) {
        fprintf(stderr, "ERROR: Could not synchronize on special protocol data\n");
    }
    
    unsigned long size = 0;
    uintptr_t *sectionData = (uintptr_t*)getsectiondata((nn_nav_mach_header *)mhp, nn_segment_name, nn_section_name, &size);
    if (size == 0) {
        pthread_mutex_unlock(&nn_nav_injectLock);
        return;
    }
...
__attribute__((constructor)) void nn_nav_initializer(void) {
    ...
    _dyld_register_func_for_add_image(nn_nav_loadSection);
}
